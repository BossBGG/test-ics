...
import {showConfirm, showError, showProgress, showSuccess} from "~/composables/alert";
  ...
  if (confirmed && uuidDeleteItem.value) {
    ...
    try {
      deleteGroupMaterialEquipment(uuidDeleteItem.value).then((res) => {
        if (res.status === 204) {
          showSuccess('ยกเลิกใบสั่งงานสร้างกลุ่มวัสดุและอุปกรณ์สำเร็จ').then(res => {
            material_equipment_table_ref.value.refresh()
          })
        } else {
          showError(res.data.message || 'ยกเลิกใบสั่งงานสร้างกลุ่มวัสดุและอุปกรณ์ไม่สำเร็จ')
        }
      })
    } catch (e) {
      console.error("Delete failed:", e);
      showError(e.message || 'ยกเลิกใบสั่งงานสร้างกลุ่มวัสดุและอุปกรณ์ไม่สำเร็จ')
    }
  }
...
async function handleToggleStatus(item: GroupMaterial) {
  ...
  showConfirm(`คุณต้องการ"${message}"การใช้งานกลุ่มวัสดุอุปกรณ์นี้ใช่หรือไม่ ?`).then((isConfirm) => {
    if (isConfirm) {
      try {
        showProgress()
        updateGroupMaterialEquipmentStatus(item.uuid, !item.isActive).then((res) => {
          if(res.data?.statusCode === 200) {
            showSuccess(`${message}การใช้งานกลุ่มวัสดุอุปกรณ์สำเร็จ`).then(res => {
              material_equipment_table_ref.value.refresh()
            })
          }else {
            showError(res.data.message || `${message}การใช้งานกลุ่มวัสดุอุปกรณ์ไม่สำเร็จ`)
          }
        })
      } catch (error) {
        showError(error.message || `${message}การใช้งานกลุ่มวัสดุอุปกรณ์ไม่สำเร็จ`);
      }
    }
  })
}
