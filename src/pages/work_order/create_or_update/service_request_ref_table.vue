<template>
  <DataTable
    ...
    :columns="columns"
    :table-api="getServiceRequestRefList"
...
import {getServiceRequestRefList, WORK_ORDER_STATUS_MAP} from "~/api/work_order_api";
...
const emit = defineEmits<{
  (e: 'update:serviceRef', value: ServiceRequest): void
}>()
...
const columns: ColumnDef<ServiceRequest>[] = [
  {
    ...
    header: "#",
    cell: ({row}) => {
      let isSelected = props.modelValue === row.original.customerRequestNo
      ...
      return h("button",
      {
          ...
          onClick: () => {
            emit("update:serviceRef", row.original)
          }
        },
        [
  ...
  },
  {
    ...
    cell: ({row}) => {
      return h("div", thaiFormatDate(new Date(row.original.createdDate.trim()), 'short'))
    },
    maxSize: 80
  }
]
