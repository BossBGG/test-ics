...
interface Props {
  modelValue?: string;
  data?: InsulatorDateData;
}

const props = defineProps<Props>();

const emit = defineEmits<{
  "update:data": [value: InsulatorDateData];
}>();
...
const updateField = (field: keyof InsulatorDateData, value: string) => {
  insulatorDateSelector.value[field] = value;
  emit("update:data", { ...insulatorDateSelector.value });
};

// คำนวณจำนวนวัน
const totalDays = computed(() => {
  if (
    !insulatorDateSelector.value.startDateTime ||
    !insulatorDateSelector.value.endDateTime
  ) {
    return 0;
  }

  const startDate = new Date(insulatorDateSelector.value.startDateTime);
  const endDate = new Date(insulatorDateSelector.value.endDateTime);
    ...
    <div>
      ...
      <div class="date-inputs-row">
        ...
        <div class="date-input-group">
          ...
          <q-input
            :model-value="insulatorDateSelector.startDateTime"
            type="datetime-local"
            outlined
        ...
        <div class="date-input-group">
          ...
          <q-input
            :model-value="insulatorDateSelector.endDateTime"
            type="datetime-local"
            outlined
            placeholder="Invalid Date"
            class="insulation-datetime-input"
