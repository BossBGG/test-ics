...
interface Props {
  modelValue?: string;
  data?: TransformerDate;
}

const props = defineProps<Props>();

const emit = defineEmits<{
  "update:data": [value: TransformerDate];
}>();
...
const transformerDateSelector = ref<TransformerDate>({
  startDateTime: props.data?.startDateTime || "",
  endDateTime: props.data?.endDateTime || "",
});

const updateField = (field: keyof TransformerDate, value: string) => {
  transformerDateSelector.value[field] = value;
  emit("update:data", { ...transformerDateSelector.value });
};

// คำนวณจำนวนวัน
const totalDays = computed(() => {
  if (
    !transformerDateSelector.value.startDateTime ||
    !transformerDateSelector.value.endDateTime
  ) {
    return 0;
  }

  const startDate = new Date(transformerDateSelector.value.startDateTime);
  const endDate = new Date(transformerDateSelector.value.endDateTime);
...
<template>
  <div>
    <div>
      <div class="font-medium mb-3">วันที่เช่าหม้อแปลง</div>
      <!-- Date Inputs Row -->
      <div class="date-inputs-row mb-4">
        ...
        <div class="date-input-group">
          ...
          <q-input
            :model-value="transformerDateSelector.startDateTime"
            type="datetime-local"
            outlined
        ...
        <div class="date-input-group">
          ...
          <q-input
            :model-value="transformerDateSelector.endDateTime"
            type="datetime-local"
            outlined
            placeholder="Invalid Date"
