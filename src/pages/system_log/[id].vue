...
import {logTypeOptions, SystemLogDataDetail} from "~/data/systemLogData";
...
import {
  getSystemLogDetail,
  getSystemLogDetailList, SystemLogDetailResponseDto, SystemLogResponseDto,
...
onMounted(async () => {
  try {
    ...
    if (response.data?.statusCode === 200 && response.data.data) {
      ...
      logType.value = logTypeOptions.filter((type) => type.value.toLowerCase() === logData.value.logType?.toLowerCase())
    } else {
      showError(response.data?.message || 'ไม่พบข้อมูล System Log').then(res => {
        handleBack()
      })
    }
  } catch (error: any) {
...
<!--                <span class="font-medium">{{ logData.userFirstName }} {{logData.userLastName}}</span>-->
          ...
          <div class="rounded-sm bg-[#FFF]">
            <div class="m-4">
              ...
              <div class="mt-1 flex items-center space-x-2">
                ...
                >{{ logType.length > 0 ? logType[0]?.label : '-' }}</span>
              </div>
            </div>
          </div>
          <div class="rounded-sm bg-[#FFF]">
          ...
          <div class="rounded-sm bg-[#FFF]">
            <div class="m-4">
              ...
              <div class="mt-1 flex items-center space-x-2">
                <span>{{ dateTimeFormatThaiMonth(new Date(logData.createdAt)) || "-" }}</span>
              </div>
            </div>
          </div>
        </div>
