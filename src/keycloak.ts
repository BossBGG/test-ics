...
export const installKeycloak: UserModule = async ({ isClient, initialState, app }) => {
      ...
      .then(async (authenticate) => {
        ...
        try {
          ...
          if (auth.data?.datas) {
            ...
            userDataStore.setUserProfile(auth.data.datas, auth.data?.y3, auth.data?.selectedPeaOffice, auth.data?.peaOffices, auth.data?.selectedPeaCode);
          }
          loading.stop()
