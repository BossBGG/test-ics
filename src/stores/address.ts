...
export const useAddressStore = defineStore('address', () => {
    ...
    try {
      if (!group) {
        ...
        await checkLoad('peaProvinces', async () => {
          const { data } = await axios.get(`addresses/provinces?group=${group}`)
  ...
  function setAddressByType(requestId) {
    addressIdCard.value = addressesByRequestId.value?.find((item) => item?.address_service_type == 1) ?? {};
    addressRequest.value = addressesByRequestId.value?.find((item) => item?.address_service_type == 2) ?? {};
    addressSendDocument.value = addressesByRequestId.value?.find((item) => item?.address_service_type == 3) ?? {};
    addressBilling.value = addressesByRequestId.value?.find((item) => item?.address_service_type == 4) ?? {};
    addressSurvey.value = addressesByRequestId.value?.find((item) => item?.address_service_type == 5) ?? {};

    const addressFormStore = useAddressFormStore();
    const { addressIdCardForm, addressRequestForm, addressSendDocumentForm, addressBillingForm, addressSurveyForm } = storeToRefs(addressFormStore);
    ...
    if (addressSendDocument.value?.id) {
      for (const key in addressSendDocument.value) {
        if (Object.prototype.hasOwnProperty.call(addressSendDocument.value, key)) {
