...
export const useUserDataStore = defineStore("userData", () => {
  ...
  function setUserProfile(
    ...
    selectedPeaCode.value = _SelectedPeaCode.value || profile?.pea_code;
  }

  async function changePeaOffice(office) {
    try {
      const data = await axios.put(`users/change-peaoffice`, { username: userProfile.value?.username, pea_office: office });
      return { status: true };
    } catch (error) {
      return { status: false };
    }
