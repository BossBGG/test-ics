...
const materialOptions: MaterialOption[] = [
  ...
  {
    code: "1010180100",
    name: "WASHER,PLAIN,SQUARE. 52X52X4.5 MM.",
    unit: "EA",
    stock: 468,
  },
  {
    code: "1020080501",
    name: "CABLE,CU,PVC,INS.&JAC. 600V, 2X10 SQ.MM.",
    unit: "M",
    stock: 192,
  },
  {
    code: "1020180001",
    name: "TAPE,PVC,O/D,0.18X19X10,000 MM. TIS.386",
    unit: "EA",
    stock: 4.75,
  },
  {
    code: "1020200000",
    name: "TAPE,ARMOR,AL 1X10 MM.",
    unit: "KG",
    stock: 10.69,
  },
  {
...
const addMaterial = () => {
  const newId =
    materialData.value.length > 0
      ? Math.max(...materialData.value.map((item) => item.id)) + 1
      : 1;
...
const getFilteredMaterialCodes = (currentId: number) => {
  const currentMaterial = materialData.value.find(
    (item) => item.id === currentId
  );
  if (currentMaterial?.equipmentName) {
    const selectedMaterial = materialOptions.find(
      (item) => item.name === currentMaterial.equipmentName
    );
...
const getFilteredEquipmentNames = (currentId: number) => {
  ...
  if (currentMaterial?.materialCode) {
    const selectedMaterial = materialOptions.find(
      (item) => item.code === currentMaterial.materialCode
    );
      ...
      <div class="rounded-[10px] border mb-4">
        <Table class="rounded-tl-[8px] rounded-tr-[8px] material-data-table">
          <TableHeader>
            <TableRow>
              <TableHead
                class="bg-[#69306D] text-white first:rounded-tl-[8px] p-2 text-center w-[80px]"
              >
                ลำดับ
              </TableHead>
              ...
              <TableHead
                class="bg-[#69306D] text-white p-2 text-center w-[140px]"
              >
                จำนวนที่เบิก
              </TableHead>
              <TableHead
              ...
              <TableHead
                class="bg-[#69306D] text-white p-2 text-center w-[100px]"
              >
                หน่วย
              </TableHead>
              <TableHead
                class="bg-[#69306D] text-white last:rounded-tr-[8px] p-2 text-center w-[140px]"
              >
              </TableHead>
            </TableRow>
          </TableHeader>
              ...
              >
                ...
                <TableCell class="p-2">
                  <q-select
                    ...
                    @update:model-value="
                      (value) =>
                        updateMaterial(material.id, 'materialCode', value)
                    "
                  />
                  <span v-else class="material-display">{{
                    material.materialCode || "ยังไม่ได้เลือก"
                  }}</span>
                </TableCell>
                ...
                <TableCell class="p-2">
                  <q-select
                    ...
                    @update:model-value="
                      (value) =>
                        updateMaterial(material.id, 'equipmentName', value)
                    "
                  />
                  <span v-else class="material-display">{{
                    material.equipmentName || "ยังไม่ได้เลือก"
                  }}</span>
                </TableCell>
                ...
                <TableCell class="p-2 text-center">
                  <q-input
                    ...
                    "
                  />
                  <span v-else class="material-quantity-text">{{
                    material.withdrawAmount
                  }}</span>
                </TableCell>
                ...
                <TableCell class="p-2 text-center">
                  <q-input
                    ...
                    @update:model-value="
                      (value) => updateMaterial(material.id, 'unit', value)
                    "
                  />
                  <span v-else class="material-display">{{
                    material.unit || "ยังไม่ได้กรอก"
                  }}</span>
                </TableCell>
