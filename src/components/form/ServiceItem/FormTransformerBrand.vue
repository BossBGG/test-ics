...
const { transformerBrands, nowService } = storeToRefs(serviceItemStore);

onMounted(async () => {
  await serviceItemStore.fetchTransformerBrand();
});

const filterValue = ref("");
const filteredMaterials = computed(() => {
  if (!filterValue.value) {
    return transformerBrands.value;
  }
  const needle = filterValue.value.toLowerCase();
  return transformerBrands.value.filter((option) => {
...
<template>
  <div :class="props.customClass">
    ...
    <q-select
      ...
      :rules="[(val) => (val && val.length > 0) || 'กรุณาเลือกยี่ห้อ']"
    >
