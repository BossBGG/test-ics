          ...
          <div class="date-range-container">
              ...
              <template v-slot:append>
                <q-icon name="event" class="cursor-pointer text-gray-400">
                  <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                    <q-date v-model="filters.from" mask="DD/MM/YYYY">
            ...
            >
              <template v-slot:append>
                <q-icon name="event" class="cursor-pointer text-gray-400">
                  <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                    <q-date v-model="filters.to" mask="DD/MM/YYYY">
                      <div class="row items-center justify-end">
                        <q-btn v-close-popup label="ปิด" color="primary" flat />
                      </div>
                    </q-date>
...
import {NewsDataFilter} from "~/data/newsData";
...
const emit = defineEmits<{
  ...
  'apply-filters': [filters: NewsDataFilter]
}>()

const isVisible = ref(props.modelValue)

const filters = reactive<NewsDataFilter>({
  from: '',
  to: '',
  status: '',
})
